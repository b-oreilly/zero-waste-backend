...
const getUserSentMessages = (req, res) => {

    Message.find({senderUserID: req.params.senderUserID}).populate('receiverUserID')
    .then((data) => {
        if (data) {
            res.status(200).json(data)
        } else {
            res.status(404).json(`No messages from ${req.params.senderUserID} found.`)
        }
    })
}

const getUserReceivedMessages = (req, res) => {

    Message.find({receiverUserID: req.params.receiverUserID}).populate('senderUserID')
    .then((data) => {
        if (data) {
            res.status(200).json(data)
        } else {
            res.status(404).json(`No messages to user ${req.params.receiverUserID} found.`)
        }
    })
}
